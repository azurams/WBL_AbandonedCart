<?php
class WBL_AbandonedCart_Model_Cron
{
//    const EMAIL_TEMPLATE_XML_PATH = 'wbl_abandonedcart/general/template';
//    const EMAIL_TEMPLATE_XML_PATH_W_COUPON = 'wbl_abandonedcart/general/coupon_template';
    ...
    public function abandoned()
    {
        ...
        {
            if(Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::ACTIVE,$storeid)) {
    ...
    protected function _proccess($store)
    {
        ...
        $days = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::DAYS, $store);
        $maxtimes = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::MAXTIMES, $store);
        $sendcoupondays = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::COUPON_DAYS, $store);
        $sendcoupon = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::SEND_COUPON, $store);
        $firstdate = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::FIRST_DATE, $store);
        $unit = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::UNIT, $store);
        $customergroups = explode(",",Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::CUSTOMER_GROUPS, $store));
        $mailsubject = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::SUBJECT, $store);
        
        if(!$days) {
            return;
        }
        if($unit == WBL_AbandonedCart_Model_Config::IN_DAYS) {
            ...
            $collection->addFieldToFilter('main_table.converted_at', array(array('null'=>true),$this->_getSuggestedZeroDate()))
                       ...
                       ->addFieldToFilter('main_table.wbl_abandonedcart_counter',array('lt' => $maxtimes));
            ...
            $collection1->addFieldToFilter('main_table.converted_at', array(array('null'=>true),$this->_getSuggestedZeroDate()))
                ->addFieldToFilter('main_table.updated_at', array('to' => $from,'from' => $firstdate))
                ->addFieldToFilter('main_table.wbl_abandonedcart_counter',array('eq' => 0));
            ...
            $collection2->addFieldToFilter('main_table.converted_at', array(array('null'=>true),$this->_getSuggestedZeroDate()))
                ->addFieldToFilter('main_table.updated_at', array('to' => $from,'from' => $firstdate))
                ->addFieldToFilter('main_table.wbl_abandonedcart_counter',array('from' => 1,'to' => $maxtimes-1));
            ...
            Mage::helper('wbl_abandonedcart')->log((string)$collection1->getSelect());
            Mage::helper('wbl_abandonedcart')->log((string)$collection2->getSelect());
...
foreach($collection as $quote) {
    Mage::log(array($quote->getUpdatedAt(),$quote->getCustomerFirstname() ,$quote->getCustomerEmail()));
}
        ...
        foreach($collection as $quote)
        {
            ...
            if($collection2->getSize()) {
                ...
                Mage::log('the customer placed an order after this abandoned cart, so we continue');
                continue;
            }

            Mage::log('Gosh, serious things here !');
            //
            //$url = Mage::getBaseUrl('web').'wbl_abandonedcart/abandoned/loadquote?id='.$quote->getEntityId();
            srand((double)microtime()*1000000);
            $token = md5(rand(0,9999999));
            $url = Mage::getModel('core/url')->setStore($store)->getUrl('',array('_nosid'=>true)).'wbl_abandonedcart/abandoned/loadquote?id='.$quote->getEntityId().'&token='.$token;

            $data = array('AbandonedURL'=>$url, 'AbandonedDate' => $quote->getUpdatedAt());
            // send email

            $senderid =  Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::SENDER, $store);
            ...
            $couponcode = '';
            if($sendcoupon && $quote2->getWblAbandonedcartCounter() + 1 == $sendcoupondays)
            {
                $templateId = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::EMAIL_TEMPLATE_XML_PATH);
                // create a new coupon
                if(Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::COUPON_AUTOMATIC)==2) {
                    ...
                    $vars = array('quote'=>$quote,'url'=>$url, 'couponcode'=>$couponcode,'discount' => $discount,
                                'todate' => $toDate, 'name' => $name);
                }
                else {
                    $couponcode = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::COUPON_CODE);
            ...
            if (!$mail->getSentSuccess()) {
                Mage::log('Mail was not sent to '. $email);
            } else {
                Mage::log('Mail was sent to '. $email);
            }               
            $translate->setTranslateInLine(true);

            $quote2->setWblAbandonedcartCounter($quote2->getWblAbandonedcartCounter()+1);
            $quote2->setWblAbandonedcartToken($token);
            $quote2->save();
            Mage::helper('wbl_abandonedcart')->saveMail('abandoned cart',$email,$name,$couponcode,$store);
    ...
    {
        $couponamount = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::COUPON_AMOUNT, $store);
        $couponexpiredays = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::COUPON_EXPIRE, $store);
        $coupontype = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::COUPON_TYPE, $store);
        $couponlength = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::COUPON_LENGTH, $store);
        $couponlabel = Mage::getStoreConfig(WBL_AbandonedCart_Model_Config::COUPON_LABEL, $store);
